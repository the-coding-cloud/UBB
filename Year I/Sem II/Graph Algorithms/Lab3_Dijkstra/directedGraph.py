...
class DirectedGraph:
    ...
    def Dijkstra(self, startVertex, endVertex):
        ...
        distance = {}
        nextNode = {}
        heappush(queue, (0, endVertex))
        distance.update({endVertex : 0})
        found = False
        
        while len(queue) > 0 and not found:
            currentCost, currentVertex = heappop(queue)
            if currentVertex == startVertex:
                found = True
                break
            
            for predecessor in self.getInboundVertices(currentVertex):
                if predecessor not in distance.keys() or\
                currentCost + self.getEdgeCost(predecessor, currentVertex) < distance[predecessor]:
                    distance.update({predecessor : distance[currentVertex] + self.getEdgeCost(predecessor, currentVertex)})
                    heappush(queue, (distance[predecessor], predecessor))
                    nextNode[predecessor] = currentVertex
                
        if not found:
            print("No path")
            return []
        else:
            print("Path length: " + str(distance[startVertex]))
            node = startVertex
            path = []
            while node != endVertex:
                path.append(node)
                node = nextNode[node]
